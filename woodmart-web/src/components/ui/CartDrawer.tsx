...
export default function CartDrawer({ open, onClose }: Props) {
  ...
  const subtotal = cart.reduce(
    (s: number, i: any) => s + (i.price || 0) * (i.qty || 0),
    0
  );
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* drawer */}
      <aside
        className="absolute right-0 top-0 w-full md:w-[420px] max-w-full bg-white shadow-xl flex flex-col
                   h-[105vh] md:h-[100vh]"
      >
        {/* header */}
        ...
        <div className="p-4 overflow-auto flex-1">
          {cart.length === 0 ? (
            ...
            cart.map((i: any) => (
              <div key={i._id} className="flex items-center gap-3 mb-4">
                <img
